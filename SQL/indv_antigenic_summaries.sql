SELECT
    ...
    ANT.lot,
    'lot' AS aggregation,
    ANT.ag_variant_hash AS identifier,
    ANT.ag_cdc_id,
    ANT.ag_isolate_id,
    AVG(ANT.titer_logfold) AS avg_logfold
FROM
    (
        SELECT
    ...
    -- Data aggregated/summarized at isolate-level
    ...
    INNER JOIN protein_modeling.relaxed_structural_distance AS RSD ON ANT.ag_variant_hash = RSD.variant_hash
...
GROUP BY
    ...
    ANT.ag_variant_hash,
    ANT.ag_cdc_id,
    ANT.ag_isolate_id -- Included SQL UNION w/ similar query to that above to aggregate data for the HINT protocol (modified data exclusions/parameters)
UNION
SELECT
    ANT.subtype,
    ANT.test_protocol,
...
FROM
    (
        ...
        GROUP BY
            ...
            ANT.sr_variant_hash,
            ANT.sr_lot
    ) AS ANT -- Data aggregated/summarized at isolate-level
    -- Example//Isolates tested with different passage histories (e.g., S1 vs. S2) for the same specimen counted distinctly
    -- Limit antigenic data consolidation based on available "relaxed" PDB structure availability/analytics
    INNER JOIN protein_modeling.relaxed_structural_distance AS RSD ON ANT.ag_variant_hash = RSD.variant_hash
    AND RSD.subtype IN ('B', 'H1', 'H3')
...
FROM
    ...
    ) AS C -- Limit antigenic data consolidation based on available "relaxed" PDB structure availability/analytics
    INNER JOIN protein_modeling.relaxed_structural_distance AS RSD ON C.ag_variant_hash = RSD.variant_hash
    AND RSD.subtype IN ('B', 'H1', 'H3')
GROUP BY
    C.subtype,
    C.test_protocol,
